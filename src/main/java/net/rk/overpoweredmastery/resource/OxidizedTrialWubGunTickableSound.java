...
public class OxidizedTrialWubGunTickableSound extends AbstractTickableSoundInstance{
    public Player player;

    public OxidizedTrialWubGunTickableSound(Player player, RandomSource randomSource) {
        super(SoundEvents.MUSIC_DISC_CREATOR.value(), SoundSource.PLAYERS, randomSource);
        this.player = player;
        this.looping = true;
        this.delay = 0;
        this.x = player.getX();
    ...
    public void tick() {
        count++;
        if(this.player.getItemInHand(player.getUsedItemHand()).is(OMItems.OXIDIZED_TRIAl_WUBS)){
